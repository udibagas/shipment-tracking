<template>
    <div>
        <el-page-header @back="$emit('back')" content="INVOICE">
        </el-page-header>
        <br />
        <el-form
            inline
        ...
        >
            <el-form-item
                ...
                v-if="
                    $store.state.auth.user.role == 21 ||
                    $store.state.auth.user.role == 31
                "
            >
                <el-button
                    size="small"
                    icon="el-icon-plus"
                    @click="openForm({ items: [] })"
                    type="primary"
                    >INVOICE BARU</el-button
                >
            </el-form-item>
            ...
            >
                <template slot-scope="scope">
                    <el-tag
                        ...
                        >{{ scope.row.status ? "FINAL" : "DRAFT" }}</el-tag
                    >
                </template>
            </el-table-column>
            <el-table-column
            ...
            >
                <template slot-scope="scope">
                    <el-tag size="small" effect="dark" type="primary"
                        >Rp {{ scope.row.total | formatNumber }}</el-tag
                    >
                </template>
            </el-table-column>
            <el-table-column
                    ...
                    <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <el-dropdown-item
                                ...
                                @click.native.prevent="print(scope.row.id)"
                                >Print Invoice</el-dropdown-item
                            >
                            <el-dropdown-item
        ...
        <InvoiceForm
            ...
            @refresh-data="requestData"
            :data="formModel"
        />

        <el-dialog
            :visible.sync="showDetail"
            :title="selectedData.number"
            fullscreen
            center
        >
            <iframe
                v-if="!!selectedData.id"
                :src="baseUrl + '/invoice/print/' + selectedData.id"
                frameborder="0"
...
export default {
    ...
    methods: {
        ...
        print(id) {
            window.open(
                BASE_URL + "/invoice/print/" + id + "?print=1",
                "_blank"
            );
        },
        openForm(data) {
        ...
        deleteData(id) {
            this.$confirm("Anda yakin akan menghapus data ini?", "Konfirmasi", {
                type: "warning",
            })
                .then(() => {
                    axios
