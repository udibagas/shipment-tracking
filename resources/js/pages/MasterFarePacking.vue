<template>
	<div>
		...
		>
			<el-form-item class="margin-bottom-10">
				<el-button
					...
					@click="openForm({})"
					type="primary"
					>TAMBAH TARIF PACKING PETI</el-button
				>
			</el-form-item>
			<el-form-item class="margin-bottom-10">
				<el-input
		...
		<el-dialog
			...
			:title="
				!!formModel.id ? 'EDIT TARIF PACKING PETI' : 'TAMBAH TARIF PACKING PETI'
			"
			width="500px"
			v-loading="loading"
...
export default {
	...
	methods: {
		...
		store() {
			this.loading = true;
			axios
				.post("/masterFarePacking", this.formModel)
				.then((r) => {
					this.showForm = false;
					this.$message({
		...
		update() {
			this.loading = true;
			axios
				.put("/masterFarePacking/" + this.formModel.id, this.formModel)
				.then((r) => {
					this.showForm = false;
					this.$message({
						message: "Data berhasil disimpan.",
