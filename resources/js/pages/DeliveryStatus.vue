<template>
    <div>
        <el-page-header @back="$emit('back')" content="DELIVERY STATUSES"> </el-page-header>
        <el-divider></el-divider>
        <el-form :inline="true" style="text-align:right" @submit.native.prevent="() => { return }">
            <el-form-item>
                <el-button @click="openForm({role: 0, password: ''})" type="primary"><i class="el-icon-plus"></i> ADD NEW DELIVERY STATUS</el-button>
            </el-form-item>
        ...
        <el-dialog :visible.sync="showForm" :title="!!formModel.id ? 'EDIT DELIVERY STATUS' : 'ADD NEW DELIVERY STATUS'" width="500px" v-loading="loading" :close-on-click-modal="false">
...
export default {
    ...
    methods: {
        ...
        store() {
            this.loading = true;
            axios.post('/deliveryStatus', this.formModel).then(r => {
                this.showForm = false;
                this.$message({
        ...
        update() {
            this.loading = true;
            axios.put('/deliveryStatus/' + this.formModel.id, this.formModel).then(r => {
                this.showForm = false
                this.$message({
                    message: 'Data berhasil disimpan.',
                    type: 'success',
        ...
        deleteData(id) {
            this.$confirm('Anda yakin akan menghapus data ini?', 'Warning', { type: 'warning' }).then(() => {
                axios.delete('/deliveryStatus/' + id).then(r => {
                    this.requestData();
                    this.$message({
        ...
        requestData() {
            ...
            this.loading = true;
            axios.get('/deliveryStatus', {params: params}).then(r => {
