<template>
    ...
    >
            ...
            <el-row :gutter="30">
                ...
                <el-col :span="8">
                    ...
                    >
                        ...
                        <div
                            class="el-form-item__error"
                            v-if="formErrors.vehicle_number"
                        >
                    ...
                    >
                        ...
                        <div
                            class="el-form-item__error"
                            v-if="formErrors.delivered_date"
                        >
                    ...
                    <el-form-item
                        label="Note"
                        :class="formErrors.note ? 'is-error' : ''"
                    >
                        <el-input
                            type="textarea"
                            rows="4"
        ...
        >
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="td-value text-right">
                        <el-checkbox
                            v-model="formModel.forwarder_cost_ppn"
                        ></el-checkbox>
                    </td>
                    <td class="td-value text-right">
                        Rp.
                ...
                <tr>
                    ...
                    <td class="td-value text-right">
                        <el-input
                            size="small"
                            v-model="formModel.additional_cost_description"
                            placeholder="Keterangan"
                        ></el-input>
                    </td>
                    <td class="td-value text-right">
                    ...
                    <td class="td-value text-right">
                        <el-checkbox
                            v-model="formModel.additional_cost_ppn"
                        ></el-checkbox>
                    </td>
                    <td class="td-value text-right">
                        Rp.
        ...
        <span slot="footer">
            <el-button
                v-if="formModel.delivery_status_id === null"
                type="primary"
                plain
                @click="save(null)"
...
export default {
    ...
    computed: {
        ...
        totalVolume() {
            return this.formModel.items.reduce((prev, curr) => {
                return (
                    prev +
                    (curr.dimension_p * curr.dimension_l * curr.dimension_t) /
                        1000000
                );
            }, 0);
        },
        ...
        totalInvoiceWeight() {
            let total = this.formModel.items.reduce((prev, curr) => {
                let invoiceWeight =
                    (curr.dimension_p * curr.dimension_l * curr.dimension_t) /
                    4000;
                return curr.weight > invoiceWeight
                    ? prev + Number(curr.weight)
                    : prev + Number(invoiceWeight);
            }, 0);

            // kalau dia reguler
            if (!!this.formModel.minimum_weight) {
                return total >= this.formModel.minimum_weight
                    ? total
                    : this.formModel.minimum_weight;
            }
        ...
        total_cost() {
            return (
                parseInt(this.delivery_cost) +
                parseInt(this.packing_cost) +
                parseInt(this.formModel.forwarder_cost) +
                parseInt(this.formModel.additional_cost) +
                this.formModel.packing_cost_ppn *
                    0.1 *
                    parseInt(this.packing_cost) +
                this.formModel.delivery_cost_ppn *
                    0.1 *
                    parseInt(this.delivery_cost) +
                this.formModel.forwarder_cost_ppn *
                    0.1 *
                    parseInt(this.formModel.forwarder_cost) +
                this.formModel.additional_cost_ppn *
                    0.1 *
                    parseInt(this.formModel.additional_cost)
            );
        },
    ...
    watch: {
        "formModel.customer_id"(v, o) {
            const customer = this.$store.state.customerList.find(
                (c) => c.id == v
            );
            if (customer) {
    ...
    methods: {
        ...
        copyItem(index, d) {
            this.formModel.items.splice(index, 0, {
                description: d.description,
                dimension_p: d.dimension_p,
                dimension_l: d.dimension_l,
                dimension_t: d.dimension_t,
                weight: d.weight,
                volume: d.volume,
                volume_weight: d.volume_weight,
                invoice_weight: d.invoice_weight,
                packing: d.packing,
                remark: d.remark,
            });
        },
        deleteItem(index, id) {
            if (!!id) {
                axios
                    .delete("/domesticDeliveryItem/" + id)
                    .then((r) => {
        ...
        save(status) {
            this.formModel.delivery_status_id = status;
            ...
            if (
                (this.formModel.service_type == "REGULER" &&
                    this.formModel.items.length == 0) ||
        ...
        store() {
            this.loading = true;
            axios
                .post("/domesticDelivery", this.formModel)
                .then((r) => {
                    this.$emit("close");
                    this.$message({
        ...
        update() {
            this.loading = true;
            axios
                .put("/domesticDelivery/" + this.formModel.id, this.formModel)
                .then((r) => {
                    this.$emit("close");
                    this.$message({
                        message: r.data.message,
        ...
        getFarePacking() {
            ...
            axios
                .get("/masterFarePacking/search", { params: params })
                .then((r) => {
