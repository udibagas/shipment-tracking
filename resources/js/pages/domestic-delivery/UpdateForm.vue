	...
	>
		<el-form label-width="150px" label-position="left">
			<el-form-item label="Status" :class="formErrors.status ? 'is-error' : ''">
				...
				>
					<el-option
						v-for="(t, i) in $store.state.deliveryStatusList.filter(
							(s) => s.id >= data.delivery_status_id
						)"
						:value="t.id"
						:label="t.name"
						:key="i"
			...
			>
				...
				></el-date-picker>
				<div class="el-form-item__error" v-if="formErrors.stt_received_date">
		...
		<div slot="footer">
			<el-button type="primary" @click="submit" icon="el-icon-success"
				>SAVE</el-button
			>
				...
				.then(() => {
					...
					axios({
						...
						url: this.formModel.id
							? `deliveryProgress/${this.formModel.id}`
							: "deliveryProgress",
					})
						.then((r) => {
							this.$message({
