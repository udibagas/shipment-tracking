<template>
    <el-card>
        <h1 style="text-align: center; font-size: 20px; margin-bottom: 10px">
            REPORT LEAD TIME PENGIRIMAN DOMESTIC
        </h1>
        <el-form inline style="text-align: center">
            <el-form-item>
                ...
                >
                    <el-option
                        v-for="y in $store.state.filterYearList
                            .map((y) => y.year)
                            .filter((y) => !!y)"
                        :key="y"
                        :value="y"
            ...
            <el-form-item v-if="$store.state.auth.user.role != 41">
                <el-select
                    v-model="customer_id"
        ...
        <el-table stripe :data="summaries">
            <el-table-column prop="month" label="Bulan" header-align="center">
            </el-table-column>
            <el-table-column label="Ontime" header-align="center">
                ...
                <el-table-column label="%" align="center" header-align="center">
                    <template slot-scope="scope">
                        {{
                            (
                                (scope.row.ontime / scope.row.total) *
                                100
                            ).toFixed(2)
                        }}%
                    </template>
            ...
            <el-table-column label="Delay" header-align="center">
                ...
                <el-table-column label="%" align="center" header-align="center">
                    <template slot-scope="scope">
                        {{
                            ((scope.row.delay / scope.row.total) * 100).toFixed(
                                2
                            )
                        }}%
                    </template>
...
export default {
    ...
    methods: {
        requestData() {
            ...
            axios
                .get("/report/leadTime", { params: params })
                .then((r) => {
                    this.chart.xAxis.data = r.data.map((d) => {
                        return this.monthsShort[d.month - 1];
                    });
                    this.chart.series[0].data = r.data.map((d) => {
                        return d.ontime;
                    });
                    this.chart.series[1].data = r.data.map((d) => {
                        return d.delay;
                    });
    ...
    mounted() {
        ...
        if (this.$store.state.auth.user.role == 41) {
            this.customer_id = this.$store.state.auth.user.customer_id;
        }
