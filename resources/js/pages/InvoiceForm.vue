  ...
  >
    ...
    <el-form label-width="150px" label-position="left">
      <el-row :gutter="30">
        ...
        <el-col :span="8">
          ...
          >
            ...
            >{{formErrors.service_type[0]}}</div>
          </el-form-item>
          <el-form-item label="Total" :class="formErrors.total ? 'is-error' : ''">
            <div
              style="font-size:20px;height:40px;line-height:40px;border:1px solid #ddd;border-radius:4px;background-color:yellow;padding-left:20px;"
            >Rp {{total | formatNumber}}</div>
      ...
      <el-form-item label="Terbilang" :class="formErrors.total_said ? 'is-error' : ''">
        <div
          style="height:40px;line-height:40px;border:1px solid #ddd;border-radius:4px;background-color:yellow;padding-left:20px;"
        >{{terbilang(total)}}</div>
    ...
    <el-table :data="formModel.items">
      ...
      <el-table-column label="Tgl Kirim" width="90" header-align="center" align="center">
        <template slot-scope="scope">{{scope.row.delivery_date | readableDateShort}}</template>
      </el-table-column>
      <el-table-column label="Tgl Terima" width="90" header-align="center" align="center">
        <template slot-scope="scope">{{scope.row.delivered_date | readableDateShort}}</template>
      </el-table-column>
      ...
      <el-table-column label="Layanan" min-width="100">
        <template
          slot-scope="scope"
        >{{scope.row.service_type == 'CHARTER' ? 'CHARTER ' + scope.row.vehicle_type : scope.row.service_type}}</template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column
        ...
        header-align="right"
      ></el-table-column>
      <el-table-column label="Tarif" width="100" align="right" header-align="right">
        <template slot-scope="scope">
          Rp. {{scope.row.fare | formatNumber}}
          <!-- <el-input type="number" v-model="scope.row.fare" size="small"></el-input> -->
        </template>
      </el-table-column>
      <el-table-column label="Biaya" prop="price" align="right" header-align="right" width="100">
        <template slot-scope="scope">Rp. {{scope.row.price | formatNumber}}</template>
      </el-table-column>
      ...
      <el-table-column label="PPN" align="right" header-align="right" width="100">
        <template slot-scope="scope">Rp. {{scope.row.tax | formatNumber}}</template>
      </el-table-column>
      <el-table-column label="Total" align="right" header-align="right" width="100">
        <template slot-scope="scope">Rp. {{scope.row.total | formatNumber}}</template>
      </el-table-column>
    </el-table>
    ...
    terbilang(v) {
      ...
      tmp.forEach(i => {
        ...
        if (array_angka[0] == "1") {
          ...
          said +=
            array_angka[0] > 0 ? bilangan[array_angka[0]] + " ratus " : "";
        }
        ...
        if (array_angka[1] == "1") {
          ...
          said +=
            array_angka[1] > 0 ? bilangan[array_angka[1]] + " puluh " : "";
          ...
          r.data.forEach(data => {
            ...
            if (d.packing_cost > 0) {
              ...
              d.description = {
                ...
                vehicle_type: d.vehicle_type ? d.vehicle_type.name : "",
                spb_number: d.spb_number,
                total_coli: d.items.filter(i => i.packing).length
              };
            ...
            if (d.additional_cost > 0) {
              ...
              d.description = {
                ...
                delivered_date: d.delivered_date,
                service_type:
                  "BIAYA LAIN - LAIN : " + d.additional_cost_description,
                origin: d.origin,
                destination: d.destination,
                vehicle_type: d.vehicle_type ? d.vehicle_type.name : "",
                spb_number: d.spb_number,
