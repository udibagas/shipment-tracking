...
| {{ $i['spb_number'] }} <br> {{ $i['resi_number'] }} | {{ $i['destination']}} <br> {{ $i['delivery_address'] }} | {{ $i['service_type'] }} <br> {{ $i['vehicle_type'] }} | {{ $i['quantity'] }} | {{ $i['invoice_weight'] }} KG | {{ $i['pick_up_date'] }} | {{ $i['etd'] }} | {{ $i['delivery_date'] }} | {{ $i['eta'] }} | {{ $i['delivered_date'] }} | {{ $i['statusName'] }} | {{ $i['updated_at'] }} |
@endforeach
@endcomponent
